...
for d in dirs:
    ...
    # os.makedirs(f"ocr/data/train/train/{base}", exist_ok=True)
    # os.makedirs(f"ocr/data/train/valid/{base}", exist_ok=True)
    # os.makedirs(f"ocr/data/train/test/{base}", exist_ok=True)
    f_name = 0
    for f in train_files:
        shutil.copy(f, f"ocr/data/train/train/{base}/{f_name}.png")
        f_name += 1
    f_name = 0
    for f in valid_files:
        shutil.copy(f, f"ocr/data/train/valid/{base}/{f_name}.png")
        f_name += 1
    f_name = 0
    for f in test_files:
        shutil.copy(f, f"ocr/data/train/test/{base}/{f_name}.png")
        f_name += 1
